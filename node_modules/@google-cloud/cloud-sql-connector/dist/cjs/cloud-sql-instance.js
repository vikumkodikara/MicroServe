...
class CloudSQLInstance {
    static async getCloudSQLInstance(options) {
        const instanceInfo = await (0, parse_instance_connection_name_1.resolveInstanceName)(options.instanceConnectionName, options.domainName);
    ...
    async performRefresh() {
        ...
        if (this.instanceInfo && this.instanceInfo.domainName) {
            try {
                const ips = await (0, dns_lookup_1.resolveARecord)(this.instanceInfo.domainName);
                if (ips && ips.length > 0) {
                    host = ips[0];
                }
            }
        ...
        if (!host) {
            host = (0, ip_addresses_1.selectIpAddress)(metadata.ipAddresses, this.ipType);
        }
